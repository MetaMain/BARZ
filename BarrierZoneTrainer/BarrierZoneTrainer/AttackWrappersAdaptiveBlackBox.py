...
#Modified from KM's ICCV Vision code to account for the adversarial class label 12/28/2021
import torch
...
#Main attack method 
def AdaptiveAttack(saveTag, device, oracle, syntheticModel, numIterations, epochsPerIteration, epsForAug, learningRate, optimizerName, dataLoaderForTraining, cleanLoader, valLoader, numClasses, epsForAttacks, clipMin, clipMax):
    ...
    advLoaderMIM = AttackWrappersWhiteBoxP.MIMNativePytorch(device, cleanLoader, syntheticModel, decayFactor, epsForAttacks, epsStep, numSteps, clipMin, clipMax, targeted = False)
    torch.save(advLoaderMIM, saveDir+"//AdvLoaderMIM")
    torch.cuda.empty_cache()
    cleanAcc = oracle.validateD(valLoader)
    robustAccMIM = 1 - oracle.evaluateAdversarialAttackSuccessRate(advLoaderMIM) #Changed line here from ICCV to account for detecting adversarial examples
...
#This is modified from the Vision ICCV paper code to account for the adversarial label
def LabelDataUsingOracle(oracle, dataLoader, numClasses):
    ...
    #Convert to hard labels and also keep track of how many samples were not labeled as adversarial
    cleanSampleCount = 0 
    yHardOracle = torch.zeros(numSamples)
    for i in range(0, numSamples):
        yHardOracle[i] = int(yPredOracle[i].argmax(axis=0))
        if yHardOracle[i] != numClasses: #Check if this sample was not marked as adversarial
            cleanSampleCount = cleanSampleCount + 1
    #Now we need to remove any null class labels, by first checking how many samples aren't null class
    xData, yWrong = DMP.DataLoaderToTensor(dataLoader) #Note we don't care about yWrong, just don't use it
    xDataFinal = torch.zeros(cleanSampleCount, xData.shape[1], xData.shape[2], xData.shape[3])
    yDataFinal = torch.zeros(cleanSampleCount)
    #Go through and save only the samples that were not marked as adversarial
    finalSampleCount = 0
    for i in range(0, numSamples):
        if yHardOracle[i] != numClasses:
            xDataFinal[finalSampleCount] = xData[i]
            yDataFinal[finalSampleCount] = yHardOracle[i]
            finalSampleCount = finalSampleCount + 1
    #Do some basic error checking
    if finalSampleCount != cleanSampleCount:
        raise ValueError("The number of cleanly labeled samples does not match the number of samples saved.")
    #Put the tensors in a dataloader and return 
    dataLoaderLabeled = DMP.TensorToDataLoader(xDataFinal, yDataFinal, transforms = None, batchSize = dataLoader.batch_size, randomizer = None)
