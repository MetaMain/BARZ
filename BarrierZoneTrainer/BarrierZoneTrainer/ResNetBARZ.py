...
class ResNetBARZ(nn.Module):
    def __init__(self, block, num_blocks, inputImageSize,  percentBiasPixels, scaleFactor, numClasses=10):
        super(ResNetBARZ, self).__init__()
        self.in_planes = 16
        ...
        forwardInputSize = 0
        if inputImageSize == 32 and num_blocks[0]==6 and (scaleFactor == 1.0 or scaleFactor == 1.25 or scaleFactor == 1.5): #Size 32 and size 48
            forwardInputSize = 256
        elif inputImageSize == 32 and num_blocks[0]==6 and (scaleFactor == 2.25 or scaleFactor == 2.5): #Size 72
            forwardInputSize = 1024
        elif inputImageSize == 32 and num_blocks[0]==6 and scaleFactor == 2.0: #Size 64
            forwardInputSize = 1024
        elif inputImageSize == 32 and num_blocks[0]==6 and scaleFactor == 3.0: #Size 96
            forwardInputSize = 2304
        elif inputImageSize == 32 and num_blocks[0]==6 and scaleFactor == 3.25:
            forwardInputSize = 2304
...
def resnet164(inputImageSize, percentBiasPixels, scaleFactor, numClasses):
    return ResNetBARZ(BasicBlock, [18, 18, 18], inputImageSize, percentBiasPixels, scaleFactor, numClasses)
